...
const DemoModal = ({ isOpen, onClose }) => {
  ...
  const businessTypes = [
    { id: 'smm', name: t.businessTypes.smm, icon: '/images/smm-agentligi.svg' },
    { id: 'beauty', name: t.businessTypes.beauty, icon: '/images/gozallik-saloni.svg' },
    { id: 'education', name: t.businessTypes.education, icon: '/images/talim-markazi.svg' },
    { id: 'auto', name: t.businessTypes.auto, icon: '/images/avtoservis.svg' },
    { id: 'restaurant', name: t.businessTypes.restaurant, icon: '/images/restoran.svg' },
    { id: 'spa_fitness', name: t.businessTypes.spa_fitness, icon: '/images/spa-fitness.svg' },
    { id: 'travel', name: t.businessTypes.travel, icon: '/images/turistik-agentlik.svg' },
    { id: 'language', name: t.businessTypes.language, icon: '/images/til-oquv-markazi.svg' },
    { id: 'advertising', name: t.businessTypes.advertising, icon: '/images/reklama-agentligi.svg' },
    { id: 'photo_event', name: t.businessTypes.photo_event, icon: '/images/fotostudiya-event.svg' },
    { id: 'other', name: t.businessTypes.other, icon: '/images/boshqa.svg' }
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && data.redirectUrl) {
        // Validate redirectUrl before opening
        if (data.redirectUrl) {
          try {
            const url = new URL(data.redirectUrl);
            const allowedHosts = ['cliento.uz', 'app.cliento.uz', 'localhost'];
  ...
  const openDemoUrl = () => {
    if (demoData?.redirectUrl) {
      try {
        const url = new URL(demoData.redirectUrl);
        const allowedHosts = ['cliento.uz', 'app.cliento.uz', 'localhost'];
        if (allowedHosts.some(h => url.hostname === h || url.hostname.endsWith('.' + h))) {
          window.open(demoData.redirectUrl, '_blank');
        }
      } catch (e) {
        // Invalid URL, ignore
      }
